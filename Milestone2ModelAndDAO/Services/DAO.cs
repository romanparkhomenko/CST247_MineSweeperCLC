...
namespace Milestone2.Services
{
    public class DAO
    {
        ...
        *                                * 
        **********************************/
        public bool Register(Milestone2.Models.User user)
        {
            //create SqlCommand to insert new user into DB
            SqlCommand cmd = new SqlCommand("INSERT INTO Users(FirstName, LastName, Username, Password) VALUES('" + user.FirstName + "', '" + user.LastName + "', '" + user.Username + "', '" + user.Password + "')", this.conn);
